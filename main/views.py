...
class AdminIndexListView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if order_by in ['vehicle_model', 'engine_model', 'transmission_model', 'drive_axle_model', 'steerable_axle_model', 'service_company']:
        ...
        if self.request.user.groups.filter(name='admin').exists() or self.request.user.groups.filter(
            ...
            for filter in filter_list:
                context[filter] = set(
                    qs_filter.filter(service_company__user=self.request.user.pk).values_list(filter + '__name', flat=True))
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.groups.filter(name='admin').exists() or self.request.user.groups.filter(
            ...
            context['maintenance_type'] = set(
                qs_filter.filter(service_company__user=self.request.user).values_list('maintenance_type__name',  flat=True))
            context['car'] = set(
                qs_filter.filter(service_company__user=self.request.user).values_list('car__vin', flat=True))
            context['service_company'] = set(
                qs_filter.filter(service_company__user=self.request.user).values_list('service_company__name', flat=True))
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.groups.filter(name='admin').exists() or self.request.user.groups.filter(
            ...
            context['recovery_method'] = set(
                qs_filter.filter(service_company__user=self.request.user).values_list('recovery_method__name', flat=True))
            context['service_company'] = set(qs_filter.values_list('service_company__name', flat=True))
